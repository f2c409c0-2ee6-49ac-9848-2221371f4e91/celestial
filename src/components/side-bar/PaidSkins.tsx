...
export function PaidSkins({ 
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.api.onAuthStateChanged((isAuth) => {
      ...
      if (isAuth) {
        ...
        fetchCustoms(); // Fetch customs when user logs in
      }
    });
  ...
  useEffect(() => {
    const handleSkinUninstalled = (event: CustomEvent) => {
      ...
      // Only refresh if the uninstalled skin is of type 'paid'
      if (type === 'paid') {
        fetchCustoms();
      }
    };
  ...
  const fetchCustoms = async () => {
    try {
      ...
      console.log('Fetching paid skins...');
      const userCustoms = await window.api.getPaidUserSkins();
      console.log('Received paid skins:', userCustoms);
      ...
      const installedSkinIds = await window.api.getInstalledSkins('paid');
      console.log('Installed paid skins:', installedSkinIds);
      
      // Filter out already installed skins
      const availableSkins = userCustoms.filter(skin => !installedSkinIds.includes(skin.id));
      console.log('Available paid skins after filtering:', availableSkins);
  ...
  const handleLogin = async () => {
    setShowLoginPanel(false);
    // The auth state change listener will handle fetching customs
  };
  ...
  useEffect(() => {
    const processQueue = async () => {
      ...
      try {
        ...
        // Install the skin
        await window.api.installSkin(installQueue[0].id, 'paid');
  ...
  const handleInstallSkin = (skinId: number) => {
    ...
    toast.info('Skin added to installation queue');
  };
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4">
        <p className="text-[#ecb96a] mb-4">Please log in to view paid skins</p>
        <Button 
  ...
  const filteredCustoms = customs.filter(custom => {
    ...
    // Check if the custom's category matches any active filter
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6 p-3 justify-items-center">
            {availableSkins.length > 0 ? (
              ...
              <div className="col-span-full flex flex-col items-center justify-center p-8 text-center">
                ...
                <p className="text-gray-400 text-sm">All paid skins have been installed or no skins match your filters</p>
              </div>
            )}
          </div>
        </ScrollArea>
